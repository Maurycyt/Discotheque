...
def eliminateFunctors(formula: CNFFormula[Term]): CNFFormula[Variable] = CNFFormula(
	formula.name, eliminateFunctors(formula.clause)
)

// Eliminates functors in a clause.
def eliminateFunctors(clause: CNFClause[Term]): CNFClause[Variable] = {
	val varNames = collectVarNames(clause)
	val (resultLiterals, resultUsedNames) = clause.literals.foldLeft((Set.empty[CNFLiteral[Variable]], varNames)) {
...
def eliminateFunctors(
	accLiterals: Set[CNFLiteral[Variable]], usedNames: Set[String], args: Vector[Term]
): (Set[CNFLiteral[Variable]], Set[String], Vector[Variable]) = {
...
def eliminateFunctor(
	accLiterals: Set[CNFLiteral[Variable]], usedNames: Set[String], arg: Term
): (Set[CNFLiteral[Variable]], Set[String], Variable) = arg match {
